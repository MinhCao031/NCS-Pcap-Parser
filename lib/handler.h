...
// print payload from pointer of char with given size (to prevent stopping at
// '\0')
void print_payload(u_char const *payload, int32_t payload_size, FILE *stream);

// print payload to match the data on wireshark
void print_hex_ascii_line(u_char const *const payload, int32_t len,
                          int32_t offset, FILE *stream);

// Get payloads of a single flow direction and concatenate them into a single

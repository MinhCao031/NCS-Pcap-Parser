  ...
  do {                                                                         \
    ...
    if (currnt_seq + currnt_len <= expect_seq) {                               \
      LOG_DBG(stream, DBG_PARSER,                                              \
              "TCP not inserted: SEQ(%u <= %u <= %u)"                          \
              " -> Payload already in flow\n",                                 \
              currnt_seq, expect_seq, currnt_seq + currnt_len);                \
    } else if (currnt_seq <= expect_seq) {                                     \
      LOG_DBG(stream, DBG_PARSER,                                              \
              "TCP inserted: New payload: SEQ(%u <= %u <= %u)\n", currnt_seq,  \
              expect_seq, currnt_seq + currnt_len);                            \
      Node *new_pkt_node =                                                     \
          create_payload_node(pkt, direction, expect_seq - currnt_seq);        \
      ...
      LOG_DBG(stream, DBG_PARSER,                                              \
              "TCP may not be inserted: SEQ(%u <= %u <= %u)\n"                 \
